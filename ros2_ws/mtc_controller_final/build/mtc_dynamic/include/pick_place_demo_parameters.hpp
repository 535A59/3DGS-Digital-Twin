#pragma message("#include \"pick_place_demo_parameters.hpp\" is deprecated. Use #include <mtc_dynamic/pick_place_demo_parameters.hpp> instead.")
...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        ...
        std::string object1_mesh_path = "";
        std::string object2_name;
        std::string object2_mesh_path = "";
  ...
  public:
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        ...
        if (param.get_name() == (prefix_ + "object1_name")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.object1_name = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "object1_mesh_path")) {
            updated_params.object1_mesh_path = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "object2_name")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.object2_name = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "object2_mesh_path")) {
            updated_params.object2_mesh_path = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "object1_name")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "";
          descriptor.read_only = false;
          auto parameter = rclcpp::ParameterType::PARAMETER_STRING;
          parameters_interface_->declare_parameter(prefix_ + "object1_name", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "object1_mesh_path")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "The absolute path to the object's mesh file (e.g., .stl, .dae).";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.object1_mesh_path);
          parameters_interface_->declare_parameter(prefix_ + "object1_mesh_path", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "object2_name")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "";
          descriptor.read_only = false;
          auto parameter = rclcpp::ParameterType::PARAMETER_STRING;
          parameters_interface_->declare_parameter(prefix_ + "object2_name", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "object2_mesh_path")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "The absolute path to the object's mesh file (e.g., .stl, .dae).";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.object2_mesh_path);
          parameters_interface_->declare_parameter(prefix_ + "object2_mesh_path", parameter, descriptor);
      }
      ...
      updated_params.object_name = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "object1_name");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'object1_name': {}", validation_result.error()));
      }
      updated_params.object1_name = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "object1_mesh_path");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.object1_mesh_path = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "object2_name");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'object2_name': {}", validation_result.error()));
      }
      updated_params.object2_name = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "object2_mesh_path");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.object2_mesh_path = param.as_string();
